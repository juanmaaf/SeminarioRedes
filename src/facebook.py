...
def estudio_facebook(ventana, excel):
    global ventana_facebook
    
    # Estudio REDES - FACEBOOK
    ...
    datos_facebook = pd.read_excel(excel, usecols=columnas_seleccionadas, header=0)
    
    # Crear una nueva ventana para mostrar el valor de las tablas
    ventana_facebook = tk.Toplevel(ventana)
    ventana_facebook.title("Seminario Redes")

    # Crear un Frame para el menú principal
    frame = tk.Frame(ventana_facebook)
    frame.pack(pady=5)
    
    # Agregar un título "Menú Principal" con letras en negrita en el Frame
    titulo = tk.Label(frame, text="Estudio Facebook", font=("Helvetica", 16, "bold"))
    titulo.pack(pady=10)

    # Método para mostrar los datos
    def mostrar_datos():
        # Convertir los datos a una cadena de texto
        texto_datos = datos_facebook.to_string(index=False)
    ...
    def analisis():
        ...
        learner = gum.BNLearner(datos_facebook)
        ...
        matriz_datos_facebook = datos_facebook.to_numpy()

        # Eliminar la última columna de la matriz
        matriz_datos_facebook = matriz_datos_facebook[:, :-1]
        ...
        for fila in matriz_datos_facebook:
        ...
        datos_facebook['DatosEstimados'] = datos_estimados

        # Guardar el DataFrame actualizado en el mismo archivo Excel
        datos_facebook.to_excel(excel, index=False)
        ...
        datos_reales = datos_facebook['C7A']
        ...
        datos_facebook['DatosTrasRed'] = datos_red    
        datos_facebook.to_excel(excel, index=False)
    ...
    def salir():
        if ventana_facebook is not None:
            ventana_facebook.destroy()
    ...
    button_analisis = tk.Button(frame, text="Análisis Facebook", command=analisis)
