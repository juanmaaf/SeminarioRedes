...
def estudio_instagram(ventana, excel):
    global ventana_instagram
    
    # Estudio REDES - INSTAGRAM
    ...
    datos_instagram = pd.read_excel(excel, usecols=columnas_seleccionadas, header=0)
    
    # Crear una nueva ventana para mostrar el valor de las tablas
    ventana_instagram = tk.Toplevel(ventana)
    ventana_instagram.title("Seminario Redes")

    # Crear un Frame para el menú principal
    frame = tk.Frame(ventana_instagram)
    frame.pack(pady=5)
    
    # Agregar un título "Menú Principal" con letras en negrita en el Frame
    titulo = tk.Label(frame, text="Estudio Instagram", font=("Helvetica", 16, "bold"))
    titulo.pack(pady=10)

    # Método para mostrar los datos
    def mostrar_datos():
        # Convertir los datos a una cadena de texto
        texto_datos = datos_instagram.to_string(index=False)
    ...
    def analisis():
        ...
        learner = gum.BNLearner(datos_instagram)
        ...
        matriz_datos_instagram = datos_instagram.to_numpy()

        # Eliminar la última columna de la matriz
        matriz_datos_instagram = matriz_datos_instagram[:, :-1]
        ...
        for fila in matriz_datos_instagram:
        ...
        datos_instagram['DatosEstimados'] = datos_estimados

        # Guardar el DataFrame actualizado en el mismo archivo Excel
        datos_instagram.to_excel(excel, index=False)
        ...
        datos_reales = datos_instagram['C7A']
        ...
        datos_instagram['DatosTrasRed'] = datos_red    
        datos_instagram.to_excel(excel, index=False)
    ...
    def salir():
        if ventana_instagram is not None:
            ventana_instagram.destroy()
    ...
    button_analisis = tk.Button(frame, text="Análisis Instagram", command=analisis)
